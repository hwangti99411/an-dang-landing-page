...
export function HeroSection({ settings }: { settings: SiteSettings }) {
  ...
  const content = {
    ...
    stats: [
      [
        settings.hero_stat_1_value,
        locale === 'vi' ? settings.hero_stat_1_label_vi : settings.hero_stat_1_label_en,
      ],
      [
        settings.hero_stat_2_value,
        locale === 'vi' ? settings.hero_stat_2_label_vi : settings.hero_stat_2_label_en,
      ],
      [
        settings.hero_stat_3_value,
        locale === 'vi' ? settings.hero_stat_3_label_vi : settings.hero_stat_3_label_en,
      ],
    ],
  };
      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-80px)] max-w-7xl gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-8">
        <div>
          <motion.div
            ...
            className="inline-flex rounded-full border border-brand-gold/20 bg-brand-gold/10 px-4 py-2 text-xs font-medium uppercase tracking-[0.22em] text-brand-gold"
          >
            {content.badge}
          </motion.div>
          <motion.h1
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
                  ...
                  <div className="mt-2 text-sm text-white/65">
                    {locale === 'vi'
                      ? 'Buổi tư vấn miễn phí để xác định nhu cầu, phạm vi và định hướng triển khai.'
                ...
                <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
                  ...
                  <div className="mt-4 text-xl font-semibold text-white">
                    {locale === 'vi' ? 'Hạ tầng hiện đại, chi phí gọn' : 'Modern stack, lean cost'}
                  </div>
                  <div className="mt-2 text-sm text-white/65">
                    {locale === 'vi'
                      ? 'Tối ưu tốc độ, khả năng mở rộng và quy trình quản trị nội dung cho doanh nghiệp.'
                      : 'Optimized for speed, scalability, and content operations for business teams.'}
                  </div>
                </div>
              </div>
            </div>
