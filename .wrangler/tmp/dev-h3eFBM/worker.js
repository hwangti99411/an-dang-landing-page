...
async function saveToSupabase(env, table, payload) {
...
async function sendToExternal(env, payload) {
...
__name(sendToExternal, 'sendToExternal');
...
app.post('/api/lead', async (c) => {
  ...
  try {
    await sendTelegram(
      c.env,
      [
        '\u{1F4E9} Lead m\u1EDBi t\u1EEB landing page An \u0110\u0103ng',
        `H\u1ECD t\xEAn: ${body.name ?? ''}`,
        `S\u0110T: ${body.phone ?? ''}`,
        `Email: ${body.email ?? ''}`,
        `C\xF4ng ty: ${body.company ?? ''}`,
        `D\u1ECBch v\u1EE5 quan t\xE2m: ${body.service_interest ?? ''}`,
...
app.post('/api/booking', async (c) => {
  ...
  try {
    await sendTelegram(
      c.env,
      [
        '\u{1F4C5} L\u1ECBch t\u01B0 v\u1EA5n m\u1EDBi t\u1EEB landing page An \u0110\u0103ng',
        `H\u1ECD t\xEAn: ${body.name ?? ''}`,
        `S\u0110T: ${body.phone ?? ''}`,
        `Email: ${body.email ?? ''}`,
        `C\xF4ng ty: ${body.company ?? ''}`,
...
// .wrangler/tmp/bundle-VPuyie/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VPuyie/middleware-loader.entry.ts
